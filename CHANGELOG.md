...
- Added Windows host-bindgen recovery. QA removes inherited `LIBCLANG_PATH`, `CLANG_PATH`, `LLVM_CONFIG_PATH`, and all `BINDGEN_EXTRA_CLANG_ARGS*` overrides before retrying, then selects a trusted host libclang from `QA_HOST_LIBCLANG_PATH`, standard 64-bit LLVM, or Visual Studio's bundled x64 LLVM. When Visual Studio C++ tools are installed but LLVM/Clang is absent, QA can provision `Microsoft.VisualStudio.Component.VC.Llvm.Clang` through the installed Visual Studio Installer before retrying.
- Package recovery now distinguishes a genuine failed eligible package from a successful package invocation with no package-owned executable LLVM regions. The latter is recorded runtime `NotApplicable` and excluded from the eligible package/source-LOC denominator in both direct and progressive finalization. Failed recovery attempts remain in the manifest, but recovered evidence can become Complete once every genuinely eligible package is measured. No coverage is invented for excluded packages.
- Added regressions for ambient Clang/bindgen sanitization, Visual Studio host-LLVM selection/provisioning arguments, dynamic no-region `NotApplicable` accounting, and recovery of unattributed members even after a successful workspace report. Complete coverage now says `COMPLETE` explicitly in the dashboard/report and shows the runtime N/A count. Advanced the visible development revision to r80; package semantic versions remain pinned at 0.1.0.
...
- Coverage scratch builds now set `CARGO_INCREMENTAL=0`, preventing disposable LLVM-coverage builds from accumulating Cargo incremental caches. Disk-full/ENOSPC diagnostics are classified from the complete child-process output as `resource-exhaustion` before the concise manifest diagnostic is truncated, so storage exhaustion cannot be hidden in the omitted middle of a long Cargo log or mislabeled as a package build defect.
- Package tail recovery now proactively bounds a shared scratch target to eight package jobs, then preserves the raw-profile count and recycles the target before continuing. A detected resource-exhaustion failure forces an immediate recycle and retries the affected package once from a fresh target; if that retry also exhausts storage, later packages still start from a clean target. This retains dependency reuse within a small batch without allowing one ever-growing target to poison every remaining workspace member. Added regressions for resource-exhaustion classification, disabled incremental scratch builds, the batch bound, and recovery-target recycling.
- The same r76 manifest also shows a separate environment prerequisite on the RocksDB-dependent packages: the original bindgen attempt reported a 4-byte/8-byte target-width mismatch and the sanitized retry reported no usable `libclang`. r77 does not hide or fabricate coverage for that genuine native-toolchain failure. Advanced the visible development revision to r77; package semantic versions remain pinned at 0.1.0.
