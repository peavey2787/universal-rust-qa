...
Standard `cargo qa` runs generate fresh progressive `cargo-llvm-cov` coverage automatically when `[coverage] mode = "auto"` (the default). Coverage starts with the project's normal/default test configuration instead of requiring `--all-features`. For workspaces, QA enumerates Cargo members, credits Cargo's actual default members when that first run succeeds, covers remaining compatible members in package groups, and goes directly to per-package isolation after a default/group failure rather than repeating an equivalent whole-workspace attempt. It retains successful raw LLVM profiles and first tries a merged report; if that shared profile/report path is empty, malformed, or unmergeable, QA independently reruns eligible packages into isolated cargo-llvm-cov target directories and merges the successful **direct JSON reports** instead. One native-build failure, WASM/target incompatibility, failing test, or bad profile therefore cannot erase trustworthy coverage from unrelated packages. Such degraded runs are explicit **Partial** coverage, and CRAP is calculated only for functions with measured line evidence; unmeasured files remain `coverage = None`. Repositories without a root `Cargo.toml` report Rust coverage **NotApplicable** rather than Failed. Other expensive/nightly backend families still require their focused command or `full`/`release`. Pass `--existing-coverage` (alias `--reuse-coverage`) when you deliberately want to reuse prior evidence, or set `[coverage] mode = "existing"` to make reuse persistent. `[coverage] mode = "off"` remains the explicit way to disable coverage.
