...
Standard `cargo qa` runs generate fresh `cargo-llvm-cov` coverage automatically when `[coverage] mode = "auto"` (the default). For ordinary implicit-host/default-feature coverage, QA first uses the same direct JSON contract that works manually: ensure `cargo-llvm-cov` is installed, run `cargo llvm-cov --ignore-run-fail --json --output-path ...` directly in the inspected workspace using the system Cargo command and cargo-llvm-cov's normal target-directory behavior, parse the resulting JSON, and retain any trustworthy measured coverage immediately. The progressive raw-profile pipeline is now fallback-only: if the direct workspace command produces no usable scoped line evidence, QA can still isolate incompatible members, preserve successful LLVM profiles, retry report merging, and finally run package-specific direct JSON recovery. Correctly scoped direct JSON remains usable even when the child command exits nonzero, while the failed attempt keeps the result explicitly **Partial**. One native-build failure, WASM/target incompatibility, failing test, or bad profile therefore cannot erase trustworthy coverage from unrelated packages. CRAP is calculated only for functions with measured line evidence; unmeasured files remain `coverage = None`. Repositories without a root `Cargo.toml` report Rust coverage **NotApplicable** rather than Failed. Other expensive/nightly backend families still require their focused command or `full`/`release`. Pass `--existing-coverage` (alias `--reuse-coverage`) when you deliberately want to reuse prior evidence, or set `[coverage] mode = "existing"` to make reuse persistent. `[coverage] mode = "off"` remains the explicit way to disable coverage.
