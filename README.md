...
Standard `cargo qa` runs generate fresh `cargo-llvm-cov` coverage automatically when `[coverage] mode = "auto"` (the default). For ordinary implicit-host/default-feature coverage, QA starts with the same direct JSON command contract that works manually: ensure `cargo-llvm-cov` is installed, run `cargo llvm-cov --json --output-path ...` from the inspected workspace with the system Cargo command, then parse and retain the resulting JSON. QA isolates coverage build artifacts in its own scratch target and sets `CARGO_INCREMENTAL=0` for those scratch builds so large workspaces do not accumulate unnecessary incremental compiler caches. If the first workspace run produces no usable JSON, QA retries with its tolerant recovery path. If a direct workspace command exits nonzero but still emits usable JSON for only part of the workspace, QA keeps that report and isolates **only the unmeasured members** instead of discarding the report or declaring untouched members failed. Packages already represented in the direct JSON are never rerun by tail recovery. Package recovery reuses compiled scratch artifacts within a bounded batch of at most eight package jobs, then recycles the scratch target before continuing. A detected disk-full/ENOSPC condition is classified as resource exhaustion from the complete child-process output, the scratch target is removed immediately, and the affected package is retried once from a fresh target; a package that still exhausts a fresh target cannot leave the scratch disk full for every later member. On Windows, bindgen/libclang failures caused by an ambient cross-compilation toolchain trigger a host-bindgen retry: QA removes inherited Clang/bindgen locator overrides, then uses `QA_HOST_LIBCLANG_PATH`, a standard 64-bit LLVM installation, or Visual Studio's bundled x64 LLVM. If Visual Studio C++ tools are present but the LLVM/Clang component is absent, QA can provision `Microsoft.VisualStudio.Component.VC.Llvm.Clang` through the installed Visual Studio Installer before retrying. A targeted package command that succeeds and emits valid LLVM JSON but has no executable line region rooted in that package under the selected test scope is recorded **NotApplicable**, not failed, and is removed from the eligible package/source-LOC denominator. The broader progressive raw-profile pipeline remains the fallback when direct JSON cannot establish usable evidence. Valid LLVM JSON is retained as numeric **Partial** evidence when genuinely eligible members still cannot be measured because of metadata, native-build prerequisites, target compatibility, tests, or profile merging. CRAP is calculated only for functions with measured line evidence; unmeasured files remain `coverage = None`. Repositories without an unambiguous Cargo workspace report Rust coverage **NotApplicable** rather than Failed. Other expensive/nightly backend families still require their focused command or `full`/`release`. Pass `--existing-coverage` (alias `--reuse-coverage`) when you deliberately want to reuse prior evidence, or set `[coverage] mode = "existing"` to make reuse persistent. `[coverage] mode = "off"` remains the explicit way to disable coverage.
...
In external-project mode coverage evidence lives under `<state>/coverage/`. `llvm-cov.json` contains the accepted line evidence and `coverage-failures.json` records the machine-readable coverage plan: package, target, feature configuration, command, exit code, failure stage/category, profile counts, and concise diagnostics for every attempt. Normal host/default-feature coverage writes `llvm-cov.json` from the direct `cargo llvm-cov --json --output-path ...` path. A parseable report is retained even when its child process exits nonzero. When that run covers only a subset of workspace members, QA performs bounded tail recovery for the missing members only, merges recovered line evidence into the canonical report, and counts a member as failed only after targeted recovery cannot obtain usable evidence. If a targeted command succeeds but LLVM emits no package-owned executable line regions, that member is runtime **NotApplicable** for the selected test scope and does not dilute scope completeness. Recovery scratch targets are temporary; their raw-profile counts and failure diagnostics are retained in the manifest before the targets are removed or recycled. Resource exhaustion is distinguished from code/build failures so a full disk cannot masquerade as dozens of independent package defects. Windows host-bindgen recovery strips inherited cross-toolchain `LIBCLANG_PATH`/Clang/bindgen overrides before selecting a known host x64 libclang; set `QA_HOST_LIBCLANG_PATH` to a trusted host `libclang.dll` directory (or the DLL itself) when an installation lives outside the standard locations. If direct JSON cannot establish usable scoped line evidence at all, QA enters the broader progressive fallback: compatible package grouping, strict/tolerant shared LLVM profile reporting, workspace direct recovery, and finally package-level isolation. Failed attempts remain in the manifest for diagnosis, but the final status can become **Complete** once every genuinely eligible package is measured and the merged LLVM evidence is healthy; unresolved eligible members keep the result **Partial**. Relative filenames in cargo-llvm-cov JSON are anchored to its absolute manifest path before package filtering, and nested workspace roots are filtered by most-specific ownership so a parent package cannot absorb an incompatible child. Reuse restores the JSON plus scope manifest. If only an old/manual `llvm-cov.json` exists, its measured function coverage remains usable but is marked **Partial** because package/source scope cannot be proven. If no trustworthy JSON can be produced, coverage stays unavailable; Universal Rust QA never fabricates a numeric value.
